...
from corpus.dataset_zh import Dataset_ZH
...
def get_corpus_target(config):
    ...
    corpus_target = corpus.corpus_zh.CorpusZH(config)
...
def get_dataset(config, id_corpus, pad_id):
    ...
    dataset_train = Dataset_ZH(id_corpus["train"], config, pad_id)
    dataset_valid = Dataset_ZH(id_corpus["valid"], config, pad_id)
    dataset_test = Dataset_ZH(id_corpus["test"], config, pad_id)
...
if __name__=='__main__':
    ## prepare config
    zh_config.process_config()
    config, _ = zh_config.get_config()
